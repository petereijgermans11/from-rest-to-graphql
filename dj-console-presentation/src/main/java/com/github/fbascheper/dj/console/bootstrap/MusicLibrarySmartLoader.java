...
public class MusicLibrarySmartLoader implements SmartLifecycle {
    ...
    public void start() {
        try {
            ...
            var artistIds = artists.stream().map(a -> a.id().value()).collect(java.util.stream.Collectors.toSet());
            for (var s : songs) {
                if (!artistIds.contains(s.artist().id().value())) {
                    throw new IllegalStateException("Song references unknown artist: " + s.title());
                }
            }
            var songIds = songs.stream().map(s -> s.id().value()).collect(java.util.stream.Collectors.toSet());
            for (var t : tracks) {
